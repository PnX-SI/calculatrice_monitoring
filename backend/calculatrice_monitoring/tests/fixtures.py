...
@pytest.fixture
def protocols_with_indicators(protocols):
    indicators = []
    ...
    flore_protocol = db.session.scalar(
        select(TMonitoringModules).where(TMonitoringModules.module_code == "FLORE")
    )
    ...
    odonate_protocol = db.session.scalar(
        select(TMonitoringModules).where(TMonitoringModules.module_code == "ODONATES")
    )
...
def calculatrice_permissions(protocols, users):
    ...
    with db.session.begin_nested():
        ...
        add_permission(users["gestionnaire"], "ODONATES", "R", "MONITORINGS_MODULES", scope=2)
